...
Рассмотрим уравнения \cref{eq_Rl.3,eq_Rl.4}. Первое из них домножим на $\sin(\Q\s)$ и продифференцируем по $\Q\s,$ а второе --- продифференцируем по $\f\s,$ а результат сложим. Получим следующее уравнение:
\begin{equation}\label{eq_Rl.3a}
L\!\left(\fr{{u\l}_r-{u\l}_2}{r\s} + \fr{\de {u\l}_2}{\de r\s}\right)\Biggr|_{r\s = R\l} = 0.
\end{equation}
 
Теперь продифференцируем по $\f\s$ уравнение \cref{eq_Rl.3} и вычтем из полученного результата уравнение \cref{eq_Rl.4}, умноженное на $\sin(\Q\s)$ и продифференцированное по $\Q\s.$ Получим следующее уравнение:
\begin{equation}\label{eq_Rl.4a}
L\!\left(\fr{\de {u\l}_3}{\de r\s} - \fr{{u\l}_3}{r\s}\right)\Biggr|_{r\s = R\l} = 0.
\end{equation}

Стоит отметить, что уравнение \cref{eq_Rl.3a} содержит только функции ${u\l}_r, {u\l}_2,$ а уравнение \cref{eq_Rl.4a} содержит только функцию ${u\l}_3.$
...
\end{split}\\
\begin{split}
\fr{2\mu\s}{\sin\Q\c}&\fr{\de^2}{\de r\c\de\f\c}\left(\fr{\P\s}{r\c}\right)+ \fr{\mu\s}{r\c^2\sin\Q\c}\fr{\de}{\de\f\c}\left(r\c^2\fr{\de^2 W}{\de r\c^2} - L(W) - 2W\right) - \\
