...
= 2\mu\s&\fr{\de^2}{\de r\s\de\Q\s}\left(\fr{\P\s}{r\s}\right)+ \fr{\mu\s}{r\s^2}\fr{\de}{\de\Q\s}\left(r^2\fr{\de^2 W}{\de r^2} - L(W) - 2W\right) + \\
...
= \fr{2\mu\s}{\sin\Q\s}&\fr{\de^2}{\de r\s\de\f\s}\left(\fr{\P\s}{r\s}\right)+ \fr{\mu\s}{r\s^2\sin\Q\s}\fr{\de}{\de\f\s}\left(r^2\fr{\de^2 W}{\de r^2} - L(W) - 2W\right) - \\
...
\begin{split}
2\mu\s&\fr{\de^2}{\de r\s\de\Q\s}\left(\fr{\P\s}{r\s}\right)+ \fr{\mu\s}{r\s^2}\fr{\de}{\de\Q\s}\left(r^2\fr{\de^2 W}{\de r^2} - L(W) - 2W\right) + \\
+ \fr{\mu\s r\s}{\sin\Q\s}&\fr{\de^2}{\de r\s\de\f\s}\left(\fr{V}{r\s}\right) = 0;
\end{split}\\
\begin{split}
\fr{2\mu\s}{\sin\Q\s}&\fr{\de^2}{\de r\s\de\f\s}\left(\fr{\P\s}{r\s}\right)+ \fr{\mu\s}{r\s^2\sin\Q\s}\fr{\de}{\de\f\s}\left(r^2\fr{\de^2 W}{\de r^2} - L(W) - 2W\right) - \\
- \mu\s r\s&\fr{\de^2}{\de r\s\de\Q\s}\left(\fr{V}{r\s}\right) = 0;
\end{split}
\end{align}
\begin{equation}
-i\om \left(\fr{\de \P\s}{\de r\s} - \fr{1}{r\s}L(W)\right) = \fr{\de \P\c}{\de r\c}.
\end{equation}
