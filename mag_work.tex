...
\begin{align*}
h_n(k\c r\c)P_n^m(\cos\Q\c)\e^{im\f\c} &= \sum_{p=0}^{\infty}\sum_{q=-p}^{p}R_{mnpq} h_p(k\c r\s)P_p^{m-q}(\cos\Q\s)\e^{i(m-q)\f\s};\\
j_n({k\s}_l r\s)P_n^m(\cos\Q\s)\e^{im\f\s} &= \sum_{p=0}^{\infty}\sum_{q=-p}^{p}Q_{mnpq} j_p({k\s}_l r\c)P_p^{q}(\cos\Q\c)\e^{iq\f\c},\\
\end{align*}
где
\begin{align*}
R_{mnpq} = 2i^{p-n}&\sum_{s=\lvert p-m\rvert}^{p+n} i^s \fr{b_p^{(nmsq)}}{N_{qs}} j_s(k\c {r\c}\s)P_s^{q}(\cos{\Q\c}\s)\e^{iq{\f\c}\s};\\
Q_{mnpq} = \fr{2i^{p-n}}{N_{qp}}&\sum_{s=\lvert p-m\rvert}^{p+n} i^s b_s^{(nmpq)} j_s({k\s}_l {r\s}\c)P_s^{m-q}(\cos{\Q\s}\c)\e^{i(m-q){\f\s}\c};\\
N_{mn}=\fr{2}{2n+1}&\fr{(n+m)!}{(n-m)!}
\end{align*}
чтобы привести разложения к одной системе координат:
